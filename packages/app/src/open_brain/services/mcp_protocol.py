"""Dependency-free bounded stdio JSON-RPC transport shared by MCP representations."""
...
class StdioMcpAdapter(Protocol):
    @property
...
def _handle_message(
    ...
    if method == "initialize":
        response = _initialize_response(request_id, params)
        return response, initialized or "result" in response
    if not initialized:
