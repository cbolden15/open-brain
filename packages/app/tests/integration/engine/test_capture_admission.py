...
def test_capture_past_the_wait_deadline_returns_durable_custody(
    ...
    try:
        queued = engine.capture.submit(_public_job_submission(engine, "admission-queue-2"))
        ...
        _finish_holder(holder)
    terminal = engine.capture.submit(_public_job_submission(engine, "admission-queue-2"))
    assert isinstance(terminal, CaptureReceipt)
    assert _count(root, "captures") == 2


def test_writer_waiters_beyond_the_cap_still_transfer_durable_custody(
    tmp_path: Path, monkeypatch: pytest.MonkeyPatch
) -> None:
    monkeypatch.setattr(capture_module, "_WRITER_WAIT_TIMEOUT_SECONDS", 5.0)
    root = tmp_path / "brain"
    ...
    try:
        ...
        queued = engine.capture.submit(_public_job_submission(engine, "admission-cap-4"))
        assert isinstance(queued, CaptureCustodyReceipt)
    finally:
        for thread in threads:
            thread.join()
    ...
    replay = engine.capture.submit(_public_job_submission(engine, "admission-cap-4"))
    assert isinstance(replay, CaptureReceipt) and replay.duplicate is True


def test_owner_submit_queues_while_a_competing_writer_holds_the_root(
    tmp_path: Path,
    ...
    with FileLease(
        root / ".open-brain", "admission-competing-writer"
    ).acquire_shared_writer():
        queued = engine.capture.submit(owner_submission)
        assert isinstance(queued, CaptureCustodyReceipt)
    assert time.monotonic() - started < 0.5
    assert _count(root, "captures") == 0
    assert _count(root, "capture_ingestion_items") == 1
    assert isinstance(engine.capture.submit(owner_submission), CaptureReceipt)
