import type { ContractDescription, RecordReadPage, RecordSummary, SearchPage } from "./client";

type Rule = {
  anyOf?: Rule[]; enum?: unknown[]; format?: "query"; idPrefixes?: string[];
  items?: Rule; max?: number; min?: number; optional?: string[];
  properties?: Record<string, Rule>; type?: "array" | "boolean" | "integer" | "null" | "object" | "string";
  unique?: boolean;
};
...
const schemas: Record<string, Rule> = {
  "contract.describe.response": { type: "object", optional: [], properties: {
    ...
    operations: { type: "array", min: 0, max: 5, unique: true, items: { type: "object", optional: [], properties: {
      ...
      dto_version: { enum: [1] }, required_grants: { type: "array", min: 1, max: 1, unique: true, items: { enum: ["search", "content-read", "history-read", "organize"] } },
    } } },
    limits: { type: "object", optional: [], properties: {
      request_bytes: { enum: [65536] }, response_bytes: { enum: [1048576] }, content_calls: { enum: [500] },
      content_bytes: { enum: [16777216] }, history_calls: { enum: [500] }, history_bytes: { enum: [16777216] },
    } },
  } },
...
export function validateT03Wire(name: keyof typeof schemas, value: unknown): Record<string, unknown> {
  try { check(schemas[name]!, value); } catch { throw "malformed_response"; }
  return value as Record<string, unknown>;
}

export function parseContractDescription(value: unknown): ContractDescription {
  return validateT03Wire("contract.describe.response", value) as unknown as ContractDescription;
}

export function parseSearchPage(value: unknown): SearchPage {
  const page = validateT03Wire("search.page.response", value);
  return { ...page, results: (page.results as unknown[]).map(parseRecordSummary) } as SearchPage;
}

export function parseRecordReadPage(value: unknown): RecordReadPage {
  const page = validateT03Wire("record.read.response", value);
  return { ...page, record: parseRecordSummary(page.record) } as RecordReadPage;
}

export function parseRecordSummary(value: unknown): RecordSummary {
  check(summary, value);
  const item = value as Record<string, unknown>;
  const source = item.provenance as RecordSummary["provenance"];
  return { ...item, provenance: { ...source, capture_ids: [...source.capture_ids] } } as RecordSummary;
}

function check(rule: Rule, value: unknown): void {
  if (rule.anyOf !== undefined) {
    for (const option of rule.anyOf) { try { check(option, value); return; } catch { /* next */ } }
  ...
  switch (rule.type) {
    ...
    case "object": {
      ...
      if ("representative_capture_id" in item) require(Array.isArray(item.capture_ids) && item.representative_capture_id === item.capture_ids[0]);
      ...
      if ("required_grants" in item) require(Array.isArray(item.required_grants) && item.required_grants[0] === grants[String(item.name)]);
...
function checkRecordConsistency(item: Record<string, unknown>): void {
  const itemProvenance = item.provenance as { capture_ids: string[] };
  if (item.record_type === "source") {
    ...
    require(itemProvenance.capture_ids.length === 1 && itemProvenance.capture_ids[0] === item.record_id);
  } else {
