...
def test_m0_manifest_binds_exact_public_adr_bytes() -> None:
    ...
    assert {item["decision_id"]: item["public_path"] for item in decisions} == (
        EXPECTED_DECISIONS
    )
...
def test_m0_public_lineage_contains_no_private_location() -> None:
    public_bytes = MANIFEST_PATH.read_bytes() + b"".join(
        (ROOT / path).read_bytes() for path in EXPECTED_DECISIONS.values()
    )
    lowered = public_bytes.lower()

    for forbidden in (b"/users/", b"caleb-brain", b"source_repository", b"source_path"):
