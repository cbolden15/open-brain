...
The local storage format is schema 7 with runtime session 2. Historical source bytes and IDs remain intact. Portable 4 exports include versioned metadata over unchanged legacy evidence. Supported Portable 1–3 imports remain available; new Portable 4 restore is excluded. Legacy connector deliveries without revision ordering and expected-head evidence refuse changed updates; ordered connector integration belongs to the later milestone.
