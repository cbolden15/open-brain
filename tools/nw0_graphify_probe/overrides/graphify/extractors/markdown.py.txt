...
from graphify.metadata import sanitize_metadata
...
def _build_link_index(root: Path) -> "dict[str, list[tuple[int, str, Path]]]":
    ...
    index dot-folders.
    """
    from graphify.discovery_rules import _SKIP_DIRS
...
def _resolve_markdown_link(raw: str, source_dir: Path,
                           wikilink: bool = False, *, scan_root: "Path | None" = None) -> "Path | None":
...
def extract_markdown(path: Path, *, scan_root: "Path | None" = None) -> dict:
    ...
    ``scan_root`` enables vault-wide wikilink resolution without importing the
    extraction orchestrator. The caller clears _MD_LINK_INDEX_CACHE per run.
    Omitting it retains standalone relative-only resolution.
    ...
    def add_link(raw: str, line: int, wikilink: bool = False) -> None:
        resolved = _resolve_markdown_link(raw, source_dir, wikilink=wikilink, scan_root=scan_root)
        if resolved is None:
            return
    ...
    for target, block_start, wikilink in references(source):
        add_link(target, block_start, wikilink=wikilink)
